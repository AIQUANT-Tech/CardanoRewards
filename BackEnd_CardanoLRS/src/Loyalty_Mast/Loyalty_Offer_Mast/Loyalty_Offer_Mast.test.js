...
import {
  ...
  deleteLoyaltyOffers,
  getLoyaltyOfferInfo,
} from "./Loyalty_Offer_Mast_Controller";
import LoyaltyOffer from "./Loyalty_Offer_Mast_Schema";
...
jest.mock("./Loyalty_Offer_Mast_Schema");
...
describe("Loyalty Offer Controller", () => {
  ...
  describe("POST /createLoyaltyOffers", () => {
    it("should create new loyalty offers", async () => {
      LoyaltyOffer.create.mockResolvedValueOnce({});
  ...
  describe("POST /deleteLoyaltyOffers", () => {
    it("should delete existing loyalty offers", async () => {
      LoyaltyOffer.deleteMany.mockResolvedValueOnce({ deletedCount: 2 });
      ...
      const payload = {
        loyalty_offer_crud_rq: {
          ...
          offer_list: [{ offer_id: "13" }, { offer_id: "14" }],
        },
      };
  ...
  describe("POST /getLoyaltyOfferInfo", () => {
    it("should fetch all loyalty offers", async () => {
      LoyaltyOffer.find.mockResolvedValueOnce([]);
      ...
      expect(response.status).toBe(200);
      expect(response.body.loyalty_offer_fetch_rs.offer_list).toBeInstanceOf(
        Array
      );
    });
  });
