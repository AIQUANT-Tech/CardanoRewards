...
import { 
  ...
  deleteLoyaltyTiers, 
  getLoyaltyTiersInfo 
} from './Loyalty_Tier_Mast_Controller.js';
...
describe('Loyalty Tier Controller Tests', () => {
  ...
  describe('Create Loyalty Tiers', () => {
    it('should create loyalty tiers successfully', async () => {
      ...
      // Simulate a successful save for each new tier.
      LoyaltyTier.prototype.save = jest.fn().mockResolvedValue({});
    ...
    it('should handle errors when creating loyalty tiers', async () => {
      // Provide a valid request structure so that the error comes from the simulated save() failure
      const mockRequest = {
        body: {
          loyalty_tier_crud_rq: {
            header: {
      ...
      // Simulate a failure when saving.
      LoyaltyTier.prototype.save = jest.fn().mockRejectedValue(new Error('Database error'));
  ...
  describe('Edit Loyalty Tiers', () => {
    it('should edit a loyalty tier successfully', async () => {
      ...
      // Simulate that the tier is found and saved successfully.
      LoyaltyTier.findOne = jest.fn().mockResolvedValue({
        save: jest.fn().mockResolvedValue({})
      });
  ...
  describe('Get Loyalty Tiers', () => {
    it('should fetch loyalty tiers successfully', async () => {
      const now = new Date();
      const mockTiers = [
        { tier_id: '1', tier_name: 'Gold', tier_desc: 'Gold Desc', modified_by: 'admin', modified_at: now, Status: 'A' },
        { tier_id: '2', tier_name: 'Silver', tier_desc: 'Silver Desc', modified_by: 'admin', modified_at: now, Status: 'I' }
      ];
      const mockRequest = { body: {} }; // For fetch, the body isn’t used beyond reading loyalty_tier_fetch_rq (which is unused)
      const mockResponse = { 
        status: jest.fn().mockReturnThis(), 
        json: jest.fn() 
      };
