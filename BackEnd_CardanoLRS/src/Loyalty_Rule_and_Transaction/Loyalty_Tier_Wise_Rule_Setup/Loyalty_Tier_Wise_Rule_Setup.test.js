...
// Optional: Suppress console.error output during tests
beforeEach(() => {
  jest.clearAllMocks();
...
describe("saveTierwiseRuleSetup", () => {
  ...
  it("should save tier-wise rules successfully", async () => {
    ...
    // Mock the save method on the LoyaltyRuleSetup instance.
    LoyaltyRuleSetup.prototype.save = jest.fn().mockResolvedValue({});

    await saveTierwiseRuleSetup(mockReq, mockRes);

    // Since there is one rule with one conversion rule, save() should be called once.
    expect(LoyaltyRuleSetup.prototype.save).toHaveBeenCalledTimes(1);
    expect(mockRes.status).toHaveBeenCalledWith(200);
    expect(mockRes.json).toHaveBeenCalledWith({
      loyalty_rule_setup_with_tier_rs: { status: "success" },
    });
  ...
  it("should handle database errors gracefully", async () => {
    mockReq = {
      body: {
        loyalty_rule_setup_with_tier_rq: {
          header: { request_type: "LOYALTY_RULE_SETUP_SAVE", user_name: "businessUser" },
          tier_wise_loyalty_rule_setup: {
            tier_wise_loyalty_rule_list: [
              {
                tier_id: 1,
    ...
    // Simulate a database error by rejecting the save() call.
    LoyaltyRuleSetup.prototype.save = jest.fn().mockRejectedValue(new Error("Database error"));
...
describe("fetchTierwiseRuleSetup", () => {
  ...
  it("should fetch tier-wise rules successfully", async () => {
    ...
    // Provide a single valid rule object.
    const mockRules = [
      {
        tier_id: 10,
        rule_desc: "Rule's Description",
        conversion_rules: [
          { min_revenue: 1001, max_revenue: 5000, percentage_rate: 8 }
        ]
      }
    ];
    ...
    LoyaltyRuleSetup.find.mockResolvedValue(mockRules);
    LoyaltyTier.findOne.mockResolvedValue(mockTierData);
  ...
  it("should handle database errors gracefully", async () => {
    LoyaltyRuleSetup.find.mockRejectedValue(new Error("Database error"));
    mockReq = { body: { loyalty_rule_setup_with_tier_rq: { header: { request_type: "FETCH_TIER_WISE_RULE_SETUP_INFO" } } } };
